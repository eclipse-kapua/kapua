...
public class DeviceImageDeleteTargetProcessor extends AbstractDeviceTargetProcessor implements TargetProcessor {
    ...
    public void processTarget(JobTarget jobTarget) throws KapuaException {
        String imageName = stepContextWrapper.getStepProperty(DeviceContainerPropertyKeys.IMAGE_NAME, String.class);
        String imageVersion = stepContextWrapper.getStepProperty(DeviceContainerPropertyKeys.IMAGE_VERSION, String.class);
        Long timeout = stepContextWrapper.getStepProperty(DeviceContainerPropertyKeys.TIMEOUT, Long.class);

        DeviceInventoryImage imageInput = deviceInventoryManagementFactory.newDeviceInventoryImage();
        imageInput.setName(imageName);
        imageInput.setVersion(imageVersion);

        KapuaSecurityUtils.doPrivileged(() -> deviceInventoryManagementService.deleteImage(jobTarget.getScopeId(), jobTarget.getJobTargetId(), imageInput, timeout));
    }
